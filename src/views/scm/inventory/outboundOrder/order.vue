<template>
  <div class="app-container">
        ...
        <el-col :span="8">
          <el-form-item label="状态" prop="state">
            <el-select v-model="form.state" clearable style="width: 100%" placeholder="状态">
              <el-option v-for="item in dict.outbound_order_status" :key="item.value" :label="item.label" :value="item.value" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="16">
    ...
    <orderItem v-if="itemVisable" :data="itemObject" @create="handleItemCreate" @update="handleItemUpdate" @cancel="handleItemCancel" />
  </div>
</template>
  ...
  methods: {
    ...
    handleNoteSave() {
      this.$refs['form'].validate(val => {
        if (!val) return
        this.saveLoad = true
        crudOutboundOrder.addOrEdit(this.form)
          .then(res => {
