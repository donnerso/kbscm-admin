  ...
  <div class="app-container">
      ...
      <crudOperation :permission="permission">
        <template slot="left">
          <el-button size="mini" type="primary" class="filter-item">
            <router-link :to="{name: 'Note', params: {}}">
              新增
            </router-link>
          </el-button>
        </template>
    ...
    <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
      ...
      <el-table-column prop="state" label="状态">
        <template slot-scope="scope">
          {{ dict.label.receipt_note_status[scope.row.state] }}
        </template>
      </el-table-column>
      ...
      <el-table-column v-permission="['admin','receiptNote:edit','receiptNote:del']" label="操作" width="150px" align="center">
        <template slot-scope="scope">
          <el-button v-permission="['admin','receiptNote:edit']" size="mini" style="margin-right: 3px;" type="text">
            <router-link v-if="scope.row" :to="{ name:'Note', params: {data: scope.row}}">
              编辑
            </router-link>
          </el-button>
          <el-popover :ref="scope.row.id" v-permission="['admin','receiptNote:del']" placement="top" width="200">
            <p>确定删除该数据吗？</p>
...
const defaultForm = { id: null, billNo: null, billDate: null, receiptType: 2, relatedBillId: '', warehouse: { id: null }, remarks: null, state: 'true' }
export default {
  name: 'ReceiptNote',
  ...
  cruds() {
    return CRUD({ title: '生产入库', url: 'api/receiptNote', sort: 'id,desc', crudMethod: { ...crudReceiptNote }})
  },
  data() {
    return {
      permission: {
