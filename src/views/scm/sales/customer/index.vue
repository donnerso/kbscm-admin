  ...
  <div class="app-container">
    ...
    <div class="head-container">
      <div v-if="crud.props.searchToggle">
        <!-- 搜索 -->
        <el-input v-model="query.name" clearable placeholder="输入客户名称搜索" style="width: 200px;" class="filter-item" @keyup.enter.native="crud.toQuery" />
    ...
    <!--表单组件-->
    <el-dialog ref="dialog" append-to-body :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="800px">
          ...
          <el-row type="flex" :gutter="15">
            <el-col :span="8">
              <el-form-item label="状态" prop="state">
                <el-radio v-for="item in dict.customer_status" :key="item.id" v-model="form.state" :label="item.value">{{ item.label }}</el-radio>
              </el-form-item>
            </el-col>
            <el-col :span="8">
    ...
    <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
      ...
      <el-table-column v-permission="['admin','customer:edit','customer:del']" label="操作" width="300px" align="center">
        <template slot-scope="scope">
          ...
          <el-button v-permission="['admin','customerAccount:list']" style="margin-left: 3px" type="text" size="mini">
            <router-link :to="'/scm/sales/customerAccount/' + scope.row.id">
              银行信息
            </router-link>
          </el-button>
          <el-button v-permission="['admin','customerInvoiceInfo:list']" style="margin-left: 3px" type="text" size="mini">
            <router-link :to="'/scm/sales/customerInvoiceInfo/' + scope.row.id">
              开票资料
            </router-link>
          </el-button>
          <el-popover
...
<script>
import crudCustomer from '@/api/scm/sales/customer'
...
export default {
  ...
  cruds() {
    return CRUD({ title: '客户', url: 'api/customer', sort: 'id,desc', crudMethod: { ...crudCustomer }})
  },
  data() {
    return {
      show: false, delLoading: false, logoArr: [],
